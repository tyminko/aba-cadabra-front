  ...
  <div class="transfer-wp-posts">
    ...
    <div class="main-section">
      <section v-for="(posts, type) in convertedPosts" :key="type">
        ...
        <div class="posts">
          <div v-for="(item, id, i) in posts" :key="id" class="item">
            <div class="counter">
              <span
                ><b>{{ i + 1 }}</b> ({{ id }})</span
              >
...
const app = {
  ...
  computed: {
    ...
    allMetaFields () {
      const metaMap = Object.values(this.postsByType).reduce(
        (metaMap, posts) => {
          const typeMetaMap = posts.reduce((res, post) => {
            if (typeof post.meta === 'object') {
              const map = Object.keys(post.meta).reduce((map, key) => {
                if (key === 'institution') {
                  console.log(
                    `%c () %c ${post.post.post_type} ${post.post.ID} meta[${key}]: `,
                    'background:#ffbb11;color:#000',
                    'color:#00aaff',
                    post.meta[key]
                  )
                }
    ...
    postsByType () {
      ...
      return Object.entries(WP).reduce((res, [key, posts]) => {
        if (key === 'attachment' || key === 'users' || key === 'page') {
          return res
        }
        res[key] = posts
        return res
    ...
    convertedPosts () {
      return Object.entries(this.postsByType).reduce((res, [type, posts]) => {
        res[type] = posts.reduce((postsOfType, postRawData) => {
          if (this.savedPosts[`wp${postRawData.post.ID}`]) {
            return postsOfType
          }
          const post = {
            status: 'public',
            ...
            modified: this.wpTimeStringToTime(
              postRawData.post.post_modified_gmt
            ),
            attachments: this.getAttachmentsFromContent(
              postRawData.post.post_content
            ),
          ...
          if (postRawData.post.post_title) {
            ...
            if (translations) {
              post.translations = this.mergePostTranslations(
                post.translations || {},
                translations
              )
            }
          ...
          if (typeof postRawData.meta === 'object') {
            Object.entries(postRawData.meta).forEach(([key, value]) => {
              switch (key) {
                ...
                case 'starts':
                  if (value) {
                    post.date = new Date(value).getTime ()
                  }
                  break
                case 'salon_date':
                  if (value) {
                ...
                case 'institution':
                  if (value) {
                    const inst = this.postsByType.institutions.find(
                      (i) => parseInt(i.post.ID) === parseInt(value)
                    )
                    if (inst) {
                      post.supportedBy = [
                        { name: inst.post.post_title, id: `wp${value}` }
                      ]
                    }
                  }
                  break
    ...
    postsToTransfer () {
      ...
      return Object.entries(this.convertedPosts).reduce(
        (res, [type, posts]) => {
          res[type] = Object.entries(posts).reduce(
            (unsavedPosts, [id, post]) => {
              ...
              if (ok) {
                const attachments = (post.attachments || []).map(
                  (attachment) => {
                    ...
                    if (savedPost.attachments) {
                      const savedId = Object.keys(savedPost.attachments).find(
                        (id) =>
                          savedPost.attachments[id].wpID === attachment.wpID
                      )
                      if (savedId) {
                        const savedA = savedPost.attachments[savedId]
                ...
                if (attachments.find((a) => !a.savedId)) {
                  unsavedPost.attachments = attachments
                  ok = false
                }
                ...
                const gallery = post.gallery.map((attachment) => {
                  ...
                  if (savedPost.gallery) {
                    const savedId = Object.keys(savedPost.gallery).find(
                      (id) => savedPost.gallery[id].wpID === attachment.wpID
                    )
                    if (savedId) {
  ...
  async created () {
    ...
    this.unsubscribe = db.collection('profiles').onSnapshot((snapshot) => {
  ...
  methods: {
    createProgrammes () {
      ...
      return Promise.all(
        Object.entries(programmes).map(([id, pr]) => {
    ...
    async upgradePosts () {
      ...
      Object.entries(this.savedPosts).forEach(([id, post]) => {
        ...
        // !!! DEBUG !!!
        console.log(
          '%c(' + id + ' - ' + post.type + ') %c toUpdate: ',
          'background:#ff00aa;color:#000',
          'color:#00aaff',
          toUpdate
        )
        ...
        if (toUpdate.partOfProgramme) {
          ...
          programmes[toUpdate.partOfProgramme.programmeId].events.push(
            programmePostRef
          )
        }
      })
    ...
    updateTotalProgress () {
      const sumProgress = Object.values(this.convertedPosts).reduce(
        (res, posts) => {
          return (
            res +
            Object.values(posts).reduce(
              (r, post) => r + (post.progress ? post.progress.total : 0),
              0
            )
          )
        },
    ...
    urlToStorageName(url) {
      ...
      return nameWithFolders
        ? nameWithFolders[0].replace(/\//g, '-').replace('jpeg', 'jpg')
        : null
    },
    ...
    allUploadedForPost(convertedPost, uploaded) {
      const attachments = [
        ...(convertedPost.attachments || []),
        ...(convertedPost.gallery || [])
      ]
      return attachments.reduce((attachmentsMap, attachment) => {
        const storageName = attachment.storageName
        const uploadedAttachment = uploaded.find(
          (a) => a.id === attachment.wpID
        )
        if (uploadedAttachment) {
          ...
          attachmentsMap[storageName] = {
            uploadedAttachment: {
              ...attachment,
              ...this.uploadedAttachments[storageName]
            },
            attachment
          }
        }
    ...
    async transferPost(type, postId) {
      const post = this.convertedPosts[type][postId]
      if (!post) {
        console.error(`Cant find Post Id: ${postId} of type: ${type}!`)
        return
      }
      ...
      try {
        ...
        this.$set(
          this.convertedPosts[type][postId].progress,
          'max',
          readyToUpload.length * 100
        )
        // !!! DEBUG !!!
        console.log(
          '%c transferPost () %c readyToUpload: ',
        ...
        const uploaded = await upload(
          ...
          (id, progress) => {
            this.$set(
              this.convertedPosts[type][postId].progress.parts,
              id,
              progress
            )
            const sum = Object.values(post.progress.parts).reduce(
              (r, p) => r + p
            )
            this.$set(
              this.convertedPosts[type][postId].progress,
              'total',
              (sum / post.progress.max) * 100
            )
            this.updateTotalProgress ()
          }
        )
        ...
        const replacements = Object.entries(allUploaded).reduce(
        ...
        attachTypes.forEach((attachType) => {
          ...
          if (post[attachType].length) {
            const attachmentsOfType = post[attachType].reduce(
              (res, attachment) => {
                let uploadedAttachment = {}
                const storageKey = allUploadedKeys.find(
                  (key) => key === attachment.storageName
                )
                // !!! DEBUG !!!
                console.log(
                  '%c () %c storageKey: ',
                ...
                if (storageKey) {
                  uploadedAttachment =
                    allUploaded[storageKey].uploadedAttachment
                  // !!! DEBUG !!!
                  console.log(
                    '%c () %c attachment: ',
                    'background:#00FF00;color:#000',
        ...
        postData.type = string.toCamel(type)
        postData = this.upgradePost(type, postData)
        const pId = `wp${wpID}`
        // !!! DEBUG !!!
        console.log(
          '%c transferPost () %c postData: ',
          'background:#ffbb00;color:#000',
          'color:#00aaff',
          postData
        )
        ...
        this.$set(this.convertedPosts[type][postId].progress, 'total', 100)

        this.processing = false
      } catch (err) {
        /* DEBUG */
        console.error(
          '%c transferPost %c, ' + type + ' ' + postId + ' err: ',
    ...
    postAuthor(wpAuthorId) {
      const profile = this.profiles.find(
        (profile) => profile.wpID === parseInt(wpAuthorId)
      )
      if (!profile) {
        return null
      }
    ...
    getAttachmentsFromContent(content) {
      let res = this.extractImagesWithWpCaptions({
        attachments: [],
        text: content
      })
      ...
      return res.attachments.map((a) => ({
        ...a,
        storageName: this.urlToStorageName(a.url)
      }))
    },
    ...
    extractABAImages({ attachments, text }) {
      const imgRegex =
        /<img.*?src='http:\/\/airberlinalexanderplatz\.de\/wp-content\/uploads\/\d{4}\/\d+\/(.*?)'.*?>/g
      let match
    ...
    extractABAFileLinks({ attachments, text }) {
      const linkRegex =
        /<a.*?href='(http:\/\/airberlinalexanderplatz\.de\/wp-content\/uploads\/\d{4}\/\d+\/(.*?))'.*?>.*?<\/a>/g
      let match
    ...
    getAttachmentByFileName(fileName) {
      return WP.attachment.find((a) => {
        if (a.meta._wp_attached_file.split('/').pop () === fileName) {
          return true
        }
        let found = false
        const m = a.meta._wp_attachment_metadata
        if (m) {
          if (m.file && m.file.split('/').pop () === fileName) {
            return true
          }
          if (m.sizes) {
    ...
    getTitleTrans(wpPost) {
      ...
      langs.forEach((lng) => {
        ...
        if (match) {
          ...
          if (lng !== 'en' && translations.en && translations.en === trans) {
            return
          }
          translations[lng] = {
            title: trans
      ...
      if (
        trasLangs.length === 1 &&
        translations[trasLangs[0]].title ===
          this.cleanupString(wpPost.post_title)
      ) {
        return null
      }
      return translations
